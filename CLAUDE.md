...
> already exist.
>
> **This IS the fn 5 MAR flag, since 2026-09-10.** "MAR" means **Make Available Resorts**
> throughout this codebase (`MAR_MAX_UNITS`, fn 5's MAR batch, the `LvcCode` wording), and fn 5's
...
GET  /api/rci-bulk-bank/units?resortCode=   Units the fn 3 grid may show at that resort, each with its fn 5 availability ranges: [{ unitNo, apartmentType, occupancy, rciReserved, bankable, bankedCount, blocks[] }]. `bankable` (rciReserved='Y' AND not a lock-off half) means new weeks can be banked; a NON-bankable unit is returned only when bankedCount > 0, so the 205 records on units fn 4 has since un-flagged (CP-PBR 3205/3206 + 3227/3228, L-10024 A8) stay visible and clearable. A unit with an empty blocks[] has no availability and is listed disabled. At a lockOnOff='Y' resort the response also carries splitTypes: string[]|null naming the excluded halves so the form can explain the omission. 400 without resortCode, 404 unknown resort (RESORT_BOOKING view)
