...
│   ├── migrate-lvc-codes.ts # LVC exchange-programme master from lvc_master.txt (fn 3 LVC Code; first 7 of 14 cols)
...
│   ├── migrate-cp-season-points.ts # Season points chart, HOME half, from ps_seasonapt.txt (fn 10; first 12 of 24 cols, Sunday=index 0)
│   ├── migrate-lvc-season-points.ts # Season points chart, AWAY half, from ps_lvcapt.txt (fn 10; first 14 of 20 cols, Sunday=index 0)
...
product picker (fn 3) and the CP Points Deduction **Charged To** picker (fn 10), both via the shared
...
Maintenance and Setup** function (`/resorts/lvc-codes`, Resorts Setup fn 3). **22 records** from `lvc_master.txt` (Informix
...
`mar` (Y/N — **NOT** from Informix, see below), `resortMgmt`, `contactPerson`, `add1-3`, `city`, `state`,
...
`legacyModUser/Date`. **`re_rci_release` intentionally NOT migrated** (business decision;
...
> ### `mar` — a Y/N flag, and NOT from Informix (reshaped 2026-09-10)
> A plain `Y`/`N` flag marking a resort as **MAR (Make Available Resorts)**, editable in fn 2
> (add form, edit form, list column, detail card). Nothing reads it yet — it is maintenance
> data, not behaviour.
>
> **It was briefly sourced from `re_exc_reg`, and that was reversed the next day.** The column
> was added on 2026-09-09 as 3-char free text migrated from `resort_mast.re_exc_reg`, whose
> width is genuinely used — across the 324 source rows: `Y` 253, `N` 36, blank 18, **`APR` 16**
> (all retired `03` resorts) and **`10`** 1 (the `%` "ALL RESORTS" pseudo-row). On review none
> of that was the wanted state, so migration `20260910090000_resort_mar_yn` **drops the column
> and re-creates it**, and `re_exc_reg` went back to being skipped. **Don't re-derive the
> sourcing** — the business set is not reproducible from that column: `V-AWT3` is an active
> partner resort that is deliberately excluded, and `L-10016` reads `Y` there but is not wanted.
>
> **`String @default("N")`, NOT NULL** — and that is only safe because the control is a
> **checkbox** (`setYN`, beside RCI Affiliate), which always submits `'Y'` or `'N'`. The `''`
> → `null` mapping in `clean()` (`resorts.controller.ts`) and in the form's payload builder can
> therefore never produce a null. Put a text input back on this field and a cleared box 400s.
>
> **zod is `z.enum(['Y','N']).default('N')`** — like `rciAffiliate` / `lockOnOff` / `paymt` but
> **without** their `.nullish()`, since null must be rejected rather than stored. The enum is
> also the 1-char constraint: the column is `TEXT`, as every string in this schema is, and there
> are no `@db.VarChar` widths anywhere. `.default('N')` covers a create that omits the field;
> on update `.partial()` wraps it in `optional()`, which short-circuits on `undefined`, so an
> unrelated edit **cannot** re-stamp `'N'` over a set flag (verified 2026-09-10).
>
> ### The MAR set lives in the IMPORTER, because this column has no source
> **`MAR_YES` in `prisma/migrate-resorts.ts`** — `V-CLC1`, `V-CLC2`, `V-LDBR`, `V-SGH`,
> `V-SGI1`, `V-SGI5` (business-supplied 2026-09-10; **6 Y / 319 N**). All six are Active
> partner/exchange resorts, on coCodes 24 / 20 / 01 / 26.
>
> Being app-owned data on a table that `migrate-table.ps1 -Table Resort` and
> `refresh-test-db.ps1` both **TRUNCATE**, the set would otherwise vanish at the next refresh.
> The const is stamped in the `createMany` payload, so a truncate-and-reload reproduces it.
> Both callers already route through the script, so **no PowerShell change was needed**.
>
> **INSERT-ONLY, deliberately unlike `ACTIVE_CODES` (`migrate-products.ts`) and `RCI_RESERVED`
> (`migrate-resort-units.ts`)**, which also `updateMany`-sweep existing rows in both directions.
> A refresh truncates first, so the insert path alone restores the baseline; a sweep would
> additionally clobber a flag staff had set, on an ordinary additive re-run. Verified
> 2026-09-10: a `Y` keyed on `L-10024` survived a direct `migrate-resorts.ts` run.
>
> **The trade-off to know:** a MAR flag keyed in fn 2 does **not** survive a truncating
> refresh unless it is added to `MAR_YES`. To change the MAR set, edit the const and re-run —
> the same rule `ACTIVE_CODES` carries.
>
> **The existing 325 rows were set by the migration itself** (an `UPDATE … WHERE "resortCode"
> IN (…)` after the `ADD COLUMN`), since the additive importer can never reach rows that
> already exist.
>
> **This IS the fn 6 MAR flag, since 2026-09-10.** "MAR" means **Make Available Resorts**
> throughout this codebase (`MAR_MAX_UNITS`, fn 6's MAR batch, the `LvcCode` wording), and fn 6's
> **Add MAR availability** picker + `createAptBlockBatch` now key off this column instead of
> inferring MAR from `coCode ∉ ('03','15','02')`. That is its only consumer — see
> *The MAR half is keyed by `Resort.mar = 'Y'`* under `### AptBlock`. Own products are still
> excluded from that batch form even when flagged, since they have real numbered apartments.
...
> `V-AWT1/3`, …) — exist for **CP Points Deduction**'s Non-Home tab (fn 10) to reference. Every
...
> **320** of the 324 resorts their apartment types (487 rows), which is what closed the fn 10
> grandfathering gap below from 407 pairs to 4.
> **The other exception is `ps_lvcapt.txt`** (fn 10's Non-Home tab, `SeasonPoint` AWAY rows), which is unfiltered and references
> **264** of the 324 resorts — that widened export is exactly what it was for. Because those resorts
> have no `ApartmentType` rows, fn 10 grandfathers apartment types already stored in its own table
> rather than requiring them in fn 4; see `### SeasonPoint`.
...
deleted** — they still satisfy the fn 10 season-points grandfathering check (which queries the
...
> **A box ticked in fn 5 does NOT survive a refresh at those four resorts** — edit `RCI_RESERVED`
> and re-run instead.
>
...
**Delete is refused 409** while the unit still has any `AptBlock` (fn 6), `ResortMaintenance` (fn 7)
...
delete would silently orphan them (still listed in fns 6/7 and RCI fn 3, still contributing to the
...
> 316 resorts, 11,689 of them on retired resorts, which is what buried the fn 5 Units screen
> (it does not filter by resort status, unlike the fn 4 Apartment Types list). `apt_block.txt`
...
**Two Add paths (2026-08-14).** Our own products (`03`/`15`/`02`) have real,
individually-numbered apartments (`A1`, `3227/3228`, `1.12A`), so they are set up one unit at a time
through **Add availability** (`POST /api/apt-blocks`), whose resort dropdown lists only those
resorts. **MAR (Make Available Resorts)** resorts allocate *N interchangeable units of a sleep type
for a period* rather than naming apartments, and go through **Add MAR availability**
...
> ### The MAR half is keyed by `Resort.mar = 'Y'`, not by coCode (2026-09-10)
> It used to be inferred as *everything that is not one of our own products*. That over-reached:
> a partner resort is not automatically made available to our members. `Resort.mar` (the fn 2
> flag — see `### Resort`) is now the criterion, and this is its **first and only consumer**.
>
> **Both halves are enforced server-side**, each with its own 400 naming the fix:
> `createAptBlockBatch` refuses an `03`/`15`/`02` resort ("use Add availability") and refuses a
> resort with `mar != 'Y'` ("tick MAR in fn 2 first"). The picker mirrors both —
> `marResorts` in `UnitsAvailability.tsx` is `mar === 'Y' && !OWN_CO_CODES.includes(coCode)`.
>
> **The own-product exclusion is deliberately kept on top of the flag** (confirmed 2026-09-10
> after being briefly removed and reinstated), and it is reachable: `V-ABC1` (a `coCode 03`
> test resort) is flagged MAR today. Filtering on `mar === 'Y'` alone would offer it in the
> picker and then always 400, since own-product resorts have real numbered apartments and must
> never take a generated `N-occupancy` batch. **A MAR flag on an `03`/`15`/`02` resort is
> therefore inert for this form** — to exercise the batch, the resort needs a partner coCode.
>
> **Consequence: `V-AWT3` (25, ABSOLUTE TWIN SANDS) left the picker** — it is an active partner
> resort with 25 MAR-shaped units and 175 availability records, but `mar = 'N'`. Its availability
> runs out **2026-12-31**, and extending it into 2027 now needs MAR ticked in fn 2 first. The
> other six flagged resorts are unaffected.

**The MAR batch keys a unit COUNT, not units**, and creates the `ResortUnit` rows *and* their
`AptBlock` records together. Body: `resortCode`, `apartmentType` (must be registered in fn 4),
...
> own fn 6 CRUD, which are in no export and can only be rebuilt by re-keying the records (680
...
- **Maintenance guard (2026-08-14):** a record cannot be **deleted** while the unit has any
  `ResortMaintenance` record overlapping its dates — **409** naming the count and pointing at fn 7.
  Without this the fn 7 rule — every maintenance range sits inside one availability
...
user picks **one of that unit's fn 6 `AptBlock` records**, and **every** date range keyed must fall
...
- **`aptBlockId` is REQUIRED on both create and update** and is a **validation input only — nothing
  ...
  the record's period.
- **The dropdown lists each fn 6 record separately, not merged.** Availability is keyed as a chain of
...
- Fed by `GET /api/apt-blocks/units?resortCode=` (`listUnitsWithAvailability`), whose React Query key
  is `['apt-blocks', 'units', resortCode]` so fn 6's own invalidate refreshes it. Units absent from
...
- **Availability is a planning ceiling**: it currently runs out at 2027-12-31 (V-LDBR, V-SGI1/5),
  ...
  uncovered window cannot be keyed until fn 6 is extended.
...
Resorts Setup fn 8), covering both national public holidays (single dates) and school breaks (date
...
points-per-night figure by **`SeasonPoint`** (fn 10) — see below. 424 rows from `ps_seasondate.txt`. Fields: `date` (plain
...
Setup fn 10) — how many points a CP member is charged **per night**, by resort × apartment type ×
season × day of week — covering both halves, discriminated by `pointsType`:

- **`HOME`** — the member's own product's resort (`coCode '02'`, CP-PBR today). `CpSeasonDate` (fn 9)
...
> superseded history (business decision: fn 10 is the only consumer and there is no booking module
...
> **Apartment types are validated against `ApartmentType` (fn 4), but existing pairs are
...
> (`allowedApartmentTypes()` in the controller). Genuinely new types still have to go through fn 4.
>
...
> SLEEP2/SLEEP4/SLEEP6 are all registered in fn 4, so home behaves exactly as before. The
...
**There is no clone action** (deliberate — unlike fns 8 and 9, which copy a year server-side).
...
> **Only `name1` is prefilled**, and it is the only one of the three that is mandatory.
> `firstName1`/`lastName1` are deliberately left **blank and optional** for staff to key if they
> want them (business decision 2026-09-09).
>
> A split at the first space was built and then **withdrawn** — do not reintroduce it without
> asking. There is no split rule that survives the data. The migrated rows put the **surname in
> `firstName1`** (`YEE` | `MIEW LING`, `KRISHNABAL` | `A/P NARAYANASAMY` — `name1 = firstName1 +
> ' ' + lastName1` on 15,630 of 17,897 rows), which is the **opposite** of what the business
> considers a first and last name; reversing it to match (`DING LIEN BING` → last `DING`, first
> `LIEN BING`) then collides with the column widths, which are sized for the older reading:
> `firstName1` is capped at 10 and the given-name half exceeds it in **14,781 of 31,973** member
> names (**46%**, longest 38), against just 3 surnames over `lastName1`'s 20. Widening or swapping
...
>
> **Six fields are mandatory on ADD only**: `rciNo`, `rciStatus`, `resortCode`,
> `renewalDate`, `expiryDate`, `name1`. Enforced by
...
> enrolment time; so are **`firstName1`/`lastName1`** (dropped from the mandatory set on
> 2026-09-09 with the split that used to prefill them — `name1` is the name that identifies the
> enrolment, so keying it into three boxes bought nothing), first/last name 2, co-owner, the
> whole mailing-address block and both phone numbers.
>
...
> key). RCI fn 1 is the only place RCI data is edited, under the `RESORT_BOOKING` matrix
> permission (it was `RESORTS_SETUP` until 2026-09-09) — so IT, Member Services and Resort
> Operations can all edit it there, and nobody can edit it from the Agreement page.
...
| Unit `rciReserved != 'Y'` | **400**, naming fn 5 |
| The week's 7 days are not covered by the unit's fn 6 `AptBlock` availability | **400**, naming the uncovered days and fn 6 |
| The unit is under `ResortMaintenance` during the week | **409**, naming fn 7 |
...
>
> **This deliberately differs from fn 7 maintenance, which still clamps.** Maintenance is an
...
> deliberately unlike fn 7. fn 7 needs the user to pick a record because ranges are keyed
...
> because fn 6 keys availability as a chain of yearly blocks and a week can legitimately
> straddle two consecutive ones (2027 wk53 runs 31-12-2027 -> 06-01-2028) - a case fn 7
...
a fn 9-style grid: pick **resort + unit + year** and every RCI week of that year is a row
...
same way fn 9 prefills an ungraded month with Silver. A year that already holds weeks is
...
fn 9 in being true for a new year with no edits, but only when something was actually
...
week on a since-un-flagged unit, or one whose fn 6 availability has lapsed) — that is the
...
> `apt-blocks.controller.ts`, which carries the same note for fn 6's delete.
...
marker is an amber ring**, not amber text as in fn 9. The single letter stays in the cell:
...
**Guards run over the CREATES only.** That is what makes a unit fn 5 has un-flagged
**clear-only for free**: blanking its weeks produces no creates, so the `rciReserved` guard
never fires, while banking a new one is refused 400 naming fn 5. No separate rule was
...
`maxWait: 15_000, timeout: 120_000`). Same semantics as fn 7's `applyMaintDelta` -
...
`applyMaintDelta` (fn 7) is still the older per-day form - extract to
...
- **`deleteResortUnit`** (fn 5) counts bulk bank rows in its 409 alongside availability and
  maintenance.
- **`deleteAptBlock`** (fn 6) refuses **409** while a banked week sits inside the record's
...
- **`createMaintenance` / `updateMaintenance`** (fn 7) refuse **409** over a banked week
...
> rule, like fn 7's availability rule, so re-flagging a unit in fn 5 must not drop history.
...
# Every Resorts Setup route below is gated SOLELY by the per-user RESORTS_SETUP_ACCESS grant --
# the RESORTS_SETUP matrix permission named on these lines is INERT and no route reads it (see
# "Restricting Resorts Setup to named users"). The grant is all-or-nothing, so the create/edit/
# delete wording below describes intent, not a check. The TWO EXCEPTIONS are the list routes
# marked "shared reference read" -- RCI reads those without the grant.
GET  /api/products?q=                       Product / company master list (q = code/name/contact; **shared reference read** - requireResortsReferenceRead - the Resorts Setup grant OR RESORT_BOOKING view: RCI fn 1's product dropdown reads it)
...
GET  /api/resorts?q=                        Resort master list (q = code/name/short name; **shared reference read** - requireResortsReferenceRead - the Resorts Setup grant OR RESORT_BOOKING view: RCI fn 1 + fn 3 resort dropdowns read it)
...
GET  /api/apt-blocks?q=&resortCode=&page=&pageSize=  Availability record list, paginated, sorted startDate desc (RESORTS_SETUP view). There is NO update endpoint - fn 6 is add-only
...
GET  /api/apt-blocks/units?resortCode=      Availability records per unit at that resort: [{ unitNo, blocks:[{id,startDate,endDate}] }], blocks newest first (400 without resortCode) - feeds the fn 7 unit picker AND its Availability dropdown; units absent from the result have no availability and are greyed out there (RESORTS_SETUP view)
...
GET  /api/resort-maintenance?q=&resortCode=&unitNo=&from=&to=&year=&month=&page=&pageSize=  Maintenance list, paginated, sorted startDate desc (RESORTS_SETUP view). year (+ optional month 1-12) filters to records whose range OVERLAPS that period; month without year is ignored. unitNo is an EXACT match and from/to (YYYY-MM-DD) an arbitrary overlap window - together they feed the fn 7 form's calendar, which greys out days the unit is already withdrawn on
...
GET  /api/rci-enrolments?q=&coCode=&rciStatus=&page=&pageSize=  RCI enrolment list, paginated (q = membership/agreement/RCI no/name/co-owner/resort code; RESORT_BOOKING view). Each row also carries memberId + agreementId, resolved per page by natural key and NOT stored, so the list can link back to Member/Agreement detail. Either may be null when the row's natural key resolves to nothing, which on the 2026-08-27 data was 4 rows (no member) and 6 (no agreement) - all six Informix source defects (a branch-code typo, a coCode typo, a `00000-` placeholder duplicate, a wrong membership, a junk `ABC`/`123` row and a legacy coCode 12), **rectified at source on 2026-09-03, so a refresh after that date should make this zero**
GET  /api/rci-enrolments/agreement-search?q=&coCode=&limit=  Agreement picker for the add form: search by membership no, member name or agreement no (the same three columns listAgreements searches). Top-N, NOT paginated - limit default 20 / cap 50, with `total` returned so the form can say "refine your search"; a `q` under 2 chars is an EMPTY 200, not a 400, since the form fetches as the user types. Each row carries memberName, memberType, nominee1Name, `suggestedName1` (the member's fullName for an INDIVIDUAL, nominee 1's for a CORPORATE - resolved server-side and pre-truncated to name1's 40 chars) and `enrolled`/`enrolmentSerialNo`, computed for the whole page in ONE batched groupBy on the full natural key. Already-enrolled agreements are returned FLAGGED, never filtered out - hiding them would read as "no such agreement". Lives here rather than on /api/agreements because that route needs the AGREEMENTS permission (RESORT_BOOKING view). **Replaced `GET /lookup`, which is gone** (RESORT_BOOKING view)
GET  /api/rci-enrolments/:id                Enrolment + memberName/acctClassify resolved by natural key (RESORT_BOOKING view)
POST /api/rci-enrolments                    Create enrolment; serialNo allocated as max+1 (RESORT_BOOKING create; 400 if the agreement key names no agreement; 409 if the agreement is already enrolled, or if the rciNo already belongs to a DIFFERENT membership - PENDING exempt, see the RCI-number rule under "### RciEnrolment")
PUT  /api/rci-enrolments/:id                Update enrolment; coCode/membershipNo/agreementNo are immutable (RESORT_BOOKING edit; 409 if rciNo is CHANGED to one owned by another membership - an edit that leaves rciNo alone is never blocked, so the 58 legacy shared numbers stay correctable)
DELETE /api/rci-enrolments/:id              Delete enrolment (RESORT_BOOKING delete; no usage guard)
GET  /api/rci-weeks?year=                    One year's RCI weeks, unpaginated, ordered weekNo asc; year REQUIRED (400 otherwise). Returns { data, year, weeks } (RESORT_BOOKING view)
GET  /api/rci-weeks/years                   Distinct years set up, newest first — feeds the year dropdown (RESORT_BOOKING view)
POST /api/rci-weeks/year                    Generate a whole year's 52/53 weeks from the year alone (body: year; RESORT_BOOKING create; 400 below MIN_YEAR 2026, 409 if the year already has weeks). Returns { year, weeks, firstFriday, lastWeekEnd }
DELETE /api/rci-weeks/year?year=            Delete a whole year's weeks (RESORT_BOOKING delete; 404 if the year is empty; **409 if any RCI fn 3 bulk bank week is banked against that calendar**, naming the count and the first 5 units - RciBulkBank has no FK to RciWeek, so this count is the only thing stopping the banked rows being stranded with no calendar behind them). There is NO per-week delete and no update endpoint at all
GET  /api/rci-bulk-bank?resortCode=&unitNo=&weekYear=  One unit's banked weeks for one year, UNPAGINATED (52/53 rows at most), ordered checkIn asc. All three params REQUIRED (400 otherwise). Rows are matched on checkIn against the year's RciWeek Friday starts, NOT on the weekYear column, so a legacy row whose weekYear never resolved still appears in the year it falls in - saveRciBulkBankYear matches identically, so the grid displays exactly what the save diffs against. Returns { data, resortCode, unitNo, weekYear } (RESORT_BOOKING view)
GET  /api/rci-bulk-bank/units?resortCode=   Units the fn 3 grid may show at that resort, each with its fn 6 availability ranges: [{ unitNo, apartmentType, occupancy, rciReserved, bankable, bankedCount, blocks[] }]. `bankable` (rciReserved='Y' AND not a lock-off half) means new weeks can be banked; a NON-bankable unit is returned only when bankedCount > 0, so the 205 records on units fn 5 has since un-flagged (CP-PBR 3205/3206 + 3227/3228, L-10024 A8) stay visible and clearable. A unit with an empty blocks[] has no availability and is listed disabled. At a lockOnOff='Y' resort the response also carries splitTypes: string[]|null naming the excluded halves so the form can explain the omission. 400 without resortCode, 404 unknown resort (RESORT_BOOKING view)
DELETE /api/rci-bulk-bank/year?resortCode=&unitNo=&weekYear=  Clear a unit's WHOLE year unconditionally + restore each day's balNight - for re-entry, since the bottom-up bin makes clearing a year cell by cell deliberately slow. NOT a diff: it also removes weeks the save would refuse to recreate (un-flagged unit, lapsed availability), and is the only bulk path that can empty a clear-only unit. All three params REQUIRED (400); 404 if the year has no calendar or the unit has nothing banked in it. **No booking guard yet** - see the TODO in deleteRciBulkBankYear (RESORT_BOOKING delete)
POST /api/rci-bulk-bank/year                Save one unit's whole year - the ONLY write path (body: resortCode, unitNo, weekYear, weeks[{ weekNo, season R|B|W or null }]). REPLACE-ALL within the year: every week the grid shows is sent, season null = not banked, and the server diffs it against what is stored into creates (blank -> season), updates (season -> season) and deletes (season -> blank). checkIn/checkOut are DERIVED from RciWeek - there is no date input anywhere. serialNos are allocated max+1..max+N inside the transaction. Requires RESORT_BOOKING create AND edit AND delete (the diff can do all three). Guards run over the CREATES only, which is what makes a non-RCI-qualified unit clear-only for free. 404 unknown resort; 400 if the unit isn't registered / a weekNo isn't in the year's fn 2 calendar / a weekNo repeats; 400 naming fn 5 if creates exist and the unit isn't rciReserved='Y'; 400 if it is a lock-off half at a lockOnOff='Y' resort. Per-week failures are COLLECTED, not thrown on the first: a week not covered by the unit's fn 6 availability (400), under maintenance (409 -> fn 7), already banked/overlapping (409), or containing a day with balNight=0 (409 - banking is refused, never clamped). ALL-OR-NOTHING - one failing week means nothing is written, and the message names the offending weeks (first 10). Returns { created, updated, deleted, clamped } (RESORT_BOOKING create+edit+delete)
...
Non-Home (10) points charts became one tabbed page at 9, leaving 10 functions; then on
**2026-09-15 LVC Code moved from 10 up to 3** - it is reference data set up alongside the company and
resort masters - pushing the old 3-9 down one to 4-10).
...
its label **prefixed with the menu number** (`6. Resorts Unit Availability/Inventory Setup`), so a
...
| 3 | Leisure Vacation Club (LVC) Code Maintenance and Setup | `/resorts/lvc-codes` | `LvcCode` |
| 4 | Apartment Sleep Types Maintenance and Setup | `/resorts/apartment-types` | `ApartmentType` |
| 5 | Apartment's Unit No. Maintenance and Setup | `/resorts/units` | `ResortUnit` |
| 6 | Resorts Unit Availability/Inventory Setup | `/resorts/availability` | `AptBlock` → `ResAvailMast` |
| 7 | Resorts Unit Under Maintenance | `/resorts/maintenance` | `ResortMaintenance` |
| 8 | Public & School Holidays Maintenance and Setup | `/resorts/holidays` | `Holiday` (tabbed: Public / School) |
| 9 | CP's Seasons Maintenance and Setup | `/resorts/seasons` | `CpSeasonDate` |
| 10 | CP Points Deduction - Maintenance and Setup | `/resorts/season-points` | `SeasonPoint` (tabbed: Home / Non-Home) |

> **Home vs non-home** is the whole distinction between the two tabs of fn 10: home = a `coCode '02'` resort
...
|---|---|---|
| Admin — Users | ✅ Done | Users, UserDetail, UserForm. No accessLevel field (removed). **Delete user** (2026-09-09) on both the list (Trash icon) and User Detail (Delete user button), gated by `canDelete('ADMIN')` — effectively IT, since every other department seeds ADMIN=NONE. Uses `ConfirmDeleteModal` + `ResultDialog` per the CRUD feedback convention. See "### Deleting a user" below. |
...
`DELETE /api/users/:id` (`deleteUser` in `users.controller.ts`, `requirePermission('ADMIN','delete')`) is a
**hard delete** of the user ID. The username and email become free for reuse.

> **The audit trail survives the account.** Migration `20260909100000_user_delete_preserve_audit` makes
> `AuditLog.userId` **nullable with `ON DELETE SET NULL`** and adds `actorUsername` / `actorName`, a
> snapshot of who performed the action, written by `writeAudit()` on every row and **backfilled** for
> existing ones. So deleting a user keeps their history and the Audit Log page still names them,
> suffixed `(deleted)`. Without this the delete was near-useless: `AuditLog.userId` was a required FK
> and 12 of the 13 users on the 2026-09-09 UAT data already had 5-564 rows each, so only a
> never-used account could ever have been removed.
>
> **`writeAudit()` now does a PK lookup of the actor** to fill the snapshot — `req.user` carries
> `username` but no `fullName`, and resolving it in the helper leaves all ~40 call sites unchanged.
> It uses the passed `tx` when there is one.
>
> **The audit `username` filter matches the snapshot OR the live relation** (`listAuditLogs`), so rows
> belonging to a deleted user stay searchable by their username.

**`UserReportAccess` splits the two directions.** `userId` is **`onDelete: Cascade`** — a deleted user's
own report grants go with them. `grantedById` is **nullable, `onDelete: SetNull`** — grants that user
**issued to other people** must not be destroyed (peter alone had issued 53). `getReportAccess` already
returned `grantedBy: … ?? null` and the UserDetail card already guards on it, so nothing else changed;
the "Granted by X" line simply disappears for those rows.

**Two guards, both 4xx:**
- **Own account** → 400 `Cannot delete your own account` (mirrors `toggleSuspend`). The button is also
  hidden for `me.id` on both screens.
- **Last active IT account** → 409. A department with `isLocked` (IT) must keep at least one ACTIVE
  user, or nobody can administer the system afterwards.

**The delete and its audit row are ONE transaction**, so a failing audit write can never leave an
account deleted with no record of who deleted it. A `P2003` from that transaction is returned as
**409** naming the problem rather than a bare 500: nothing should reference a user with a RESTRICT FK
once the migration is applied, so in practice it means the migration has NOT reached that database.
That is exactly how this first failed on 2026-09-09 — the code was deployed without `-MigrateDb`, so
`UserReportAccess_userId_fkey` was still RESTRICT and deleting a user holding report grants 500'd.

**Delete vs suspend:** suspend (`PATCH /:id/suspend`) is still the way to retire a real member of staff —
it bars login and keeps the ID resolvable everywhere. Delete is for removing an ID outright (mistaken
creates, duplicated test accounts). The confirm modal says so.

### Cross-module API permissions

**A page in one module must not depend on another module's permission.** When a screen needs data
that "belongs" to another module, give it an endpoint under its **own** module's permission rather
than reusing the other module's route:

| Endpoint | Lives under | Instead of |
|---|---|---|
| `GET /api/members/enquiry` | `MEMBERS` | `/api/agreements` (needs `AGREEMENTS`) |
| `GET /api/rci-enrolments/agreement-search` | `RESORT_BOOKING` | `/api/agreements` (needs `AGREEMENTS`) |

> **The one sanctioned exception is shared REFERENCE data**, where duplicating the endpoint would
> also duplicate the hook and its React Query cache key for no benefit. `GET /api/resorts` and
> `GET /api/products` are read by both Resorts Setup and RCI, which sit on different permissions, so
> they take **`requireResortsReferenceRead()`** — it passes for the Resorts Setup grant holder or
> for `RESORT_BOOKING` view, reachable from either audience and widening neither. See
> "### Restricting Resorts Setup to named users" below.
>
> If a third module ever needs them, widen that middleware — do not reach for another module's
...
- **`ReportKey` enum**: `MEMBER_REPORT`, `AGREEMENT_REPORT`, `EXPIRY_REPORT`, `EXPIRING_MEMBER_REPORT`, `REMAINING_VALUE_REPORT`, `EXPIRY_SUMMARY_REPORT`, `PBS_PAY_BY_MONTH_REPORT`, `PBS_CLAIM_REPORT`, `PBS_NOT_IN_PBS_REPORT`, `PBS_VARIANCE_REPORT`, `PBS_AUTO_TRANSFER`, `RESORTS_SETUP_ACCESS` — add new values here when adding reports. `PBS_AUTO_TRANSFER` gates the **Auto Transfer to Claim** process (not a report — a maintenance function) via the same per-user grant model: its routes (`/api/pbs/transfer*`) use `requireReportAccess('PBS_AUTO_TRANSFER')` instead of `requirePermission('PBS_SCHEME', …)`, and the menu item in `Pbs.tsx` is gated by `hasReport('PBS_AUTO_TRANSFER')`.
- **`RESORTS_SETUP_ACCESS` gates a whole MODULE** (2026-09-09), the same way `PBS_AUTO_TRANSFER`
  gates a function: it **replaces** `requirePermission`, it does not sit alongside it. All 10
  Resorts Setup routers carry `requireResortsSetupAccess` and **no `requirePermission` call at
  all**, so the `RESORTS_SETUP` department-matrix row is inert and the Departments screen hides it.
  See "### Restricting Resorts Setup to named users" below.
...
### Restricting Resorts Setup to named users

**Resorts Setup (fns 1-10) needs the `RESORTS_SETUP` department permission AND the per-user
`RESORTS_SETUP_ACCESS` grant. RCI (fns 1-3) runs on `RESORT_BOOKING` instead.** Both landed together
on 2026-09-09.

**Why.** `RESORTS_SETUP` gated two unrelated areas — resort master data (fns 1-10) and RCI member
enrolment (fns 1-3) — and the business wanted **one** Member Services user (jennifer) doing Resorts
Setup while her three colleagues kept RCI. Permissions are per-**department**, so the matrix alone
cannot separate four users who share one department row.

- **RCI moved to `RESORT_BOOKING`**, a pre-existing `AppModule` value that **no route had ever
  used** (only a "coming soon" sidebar placeholder). So this half needed **no migration and no new
  enum variant** — the 2026-07-23 login-breaking footgun is still avoided — and no permission edit
  either, since IT, Member Services and Resort Operations already had it FULL. This also fixes the
  underlying modelling error of two unrelated areas sharing one permission.
- **Resorts Setup gained the grant, and the grant is the SOLE gate.**
  `requireResortsSetupAccess` (from `middleware/permissions.ts`) is on `router.use` for 8 routers
  and after the shared read on the other 2 (below); **no Resorts Setup route calls
  `requirePermission` any more**. Grant/revoke from **Admin → Users → Report & Function Access**;
  IT bypasses it as it bypasses everything.

> ### The RESORTS_SETUP department-matrix row is INERT, and hidden (2026-09-09)
> It was briefly `AND`ed with the grant — matrix says *what*, grant says *who* — and that was
> unmanageable in the Departments screen: ticking the module granted nothing on its own, while
> unticking it silently revoked an already-granted user, and a **newly created department bootstraps
> all-deny**, so a grant issued to someone in it would not have worked. Four dead-looking checkboxes
> that could still take access away.
>
> `RESORTS_SETUP` is therefore **removed from `MODULES` in `pages/admin/Departments.tsx`**, which
> shows an amber note pointing at the user profile instead. The DB rows still exist and are simply
> never read. Three things must stay in step if this is ever revisited:
> - `requireResortsSetupAccess` in `middleware/permissions.ts` — the only backend gate;
>   `requirePermission`'s docblock states that RESORTS_SETUP never reaches it.
> - `AuthContext.tsx` — all four `can*` helpers special-case `RESORTS_SETUP` and return the grant,
>   so the ~40 `can*('RESORTS_SETUP')` call sites in the Resorts Setup pages need no edits and keep
>   reading naturally.
> - `MODULES` in `Departments.tsx` — the row must not come back.
>
> **The grant is ALL-OR-NOTHING**: there is no view/create/edit/delete split for Resorts Setup, so a
> granted user has full access including delete. That is why the route files carry no per-action
> guards — don't add `requirePermission` back to them expecting it to do something.
> `RESORT_BOOKING` is relabelled **"Resort Booking / RCI"** in that matrix, since ticking it is what
> grants RCI.

> ### `GET /api/resorts` and `GET /api/products` are deliberately NOT behind the grant
> They are **shared reference data**: RCI fn 1 reads both (`useRciResorts`, `useActiveProducts`)
> and RCI fn 3 reads resorts (`useActiveResorts`), for their dropdowns — and the Resorts Setup pages
> read them too. Both list routes use **`requireResortsReferenceRead()`**, which passes for anyone
> holding the Resorts Setup grant **or** whose department has `RESORT_BOOKING` view. Each is
> declared **before** its router's `router.use(requireResortsSetupAccess)` line, which is what keeps
> the guard off it — move it below and the RCI dropdowns go empty.
>
> **Do not move them below the guard.** Everything else in `resorts.ts` / `products.ts` sits under
> `requireResortsSetupAccess`, `GET /api/resorts/:id` included (its only caller is `ResortDetail`,
> fn 2). Getting this wrong is **silent**: the hooks have no `onError`, so a 403 just leaves the
> dropdowns empty with nothing on screen to explain it. Neither route is *widened* by this — the
> reachable set is the same as before.

**Frontend.** `RequireAccess` in `components/RequirePermission.tsx` is a **pathless layout-route
guard** (`<Outlet/>`) wrapping the 14 `resorts/*` and 4 `rci/*` routes in `App.tsx` — previously
neither module had **any** route guard, so a user without permission rendered the page shell and
collected 403s. It redirects to an absolute `"/"`, **not** `RequireEdit`'s `".."`: a pathless route
contributes no URL segment, so relative resolution there is fragile. It also covers the two landing
pages, which have no gating of their own — **don't add per-page `canView` checks**. The sidebar's
Resorts group holds only Resorts Setup (`canView('RESORTS_SETUP') && hasReport('RESORTS_SETUP_ACCESS')`);
a **Booking** group holds RCI plus the Resort Booking placeholder, replacing the old "Coming Soon"
group.

**Two consequences worth knowing:**
- **`Resort Operations`' seeded `RESORTS_SETUP: FULL` now means nothing** — as does every other
  department's. It has no users today, but `prisma/seed.ts` and the prose above still present it as
  a Resorts Setup owner; any future user there needs the grant issued individually.
- **Cloning a user copies their grants** (`cloneUser` in `users.controller.ts` copies
  `reportAccess`), so cloning jennifer to spin up a new Member Services account hands out Resorts
  Setup. Revoke it on the clone.

**The new key must NOT be added** to `REPORT_LIST` in `pages/reports/Reports.tsx` or to the
`reportItems` OR-list in `Sidebar.tsx:67` — it would put a module gate on the `/reports` card grid
and light up a Reports link for whoever holds it.

**Adding a new report — checklist:**
1. Add the new key to `ReportKey` enum in `prisma/schema.prisma` → `npx prisma migrate dev`
2. Add to `ALL_REPORT_KEYS` + `REPORT_LABELS` in `backend/src/controllers/reports/access.controller.ts` — and **if the key grants a FUNCTION rather than a report, add it to `FUNCTION_KEYS` in the same file**, which is what puts it on the Functions tab of the User Detail card (see below)
...
**The User Detail card is tabbed: Reports | Functions** (2026-09-09). `getReportAccess` serves a
`kind` of `'REPORT' | 'FUNCTION'` per entry, derived from **`FUNCTION_KEYS`** in
`access.controller.ts` — the two function keys today are `PBS_AUTO_TRANSFER` (the Auto Transfer to
Claim process) and `RESORTS_SETUP_ACCESS` (the sole gate for the Resorts Setup module), so the split
is **10 reports / 2 functions**. The frontend **must not re-derive the split** — it filters on
`kind` — or a new key would land on the wrong tab in one place and not the other. Each tab numbers
its own rows from 1 and shows a `granted/total` count in the tab label; the tab is local state, not
a URL param, because it is a card on a detail page rather than a list view whose results Back should
restore.
