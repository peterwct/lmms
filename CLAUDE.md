...
> The 317 new resorts have no units, availability grid or maintenance, so the
> Resorts Availability chart (which builds its rows from `ResAvailMast`) is unaffected.
...
{ status: 'A' } }` in `listApartmentTypes`, which narrows 487 rows to **19 across 13 resorts**.
